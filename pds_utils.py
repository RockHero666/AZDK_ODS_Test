...
from azdk.azdkdb import AzdkDB, WState
from azdk.azdksocket import AzdkSocket, ServerCmd, PDSServerCmd, AzdkServerCmd, PDSServerState, send_azdk_cmd
...
def pds_find_focus_s():
    ...
    # PDS commands
    pcmdSetFocus = ServerCmd(PDSServerCmd.SET_FOCUS)
    ...
    while not send_azdk_cmd(azs, cmdSetupNotif, cmdTimeout): pass
    # AZDK set idle mode
    while not send_azdk_cmd(azs, cmdSetIdleMode, cmdTimeout): pass
    # AZDK change flag
    while not send_azdk_cmd(azs, cmdChangeFlags, cmdTimeout): pass
    # AZDK set auto mode
    send_azdk_cmd(azs, cmdSetAutoMode, cmdTimeout)
