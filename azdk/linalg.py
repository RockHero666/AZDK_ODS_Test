...
class Rotator:
    def __init__(self, _vspd=None, _qs=None):
    ...
    def __str__(self):
        s = 'Qz=' + str(self.qs)
        s += f', Vr={self.spd:4.2f}*' + str(self.dir)
        return s

    def rotation(self, t: float):
        q = Quaternion(self.dir, self.spd * t)
        q *= self.qs
        if q.w < 0: q.inverse()
        return q
